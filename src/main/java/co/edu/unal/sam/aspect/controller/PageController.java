...
public class PageController {
    ...
    public ResponseEntity<Void> createPage(@Valid @RequestBody Page page) {
        ...
        final URI newPageUri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
    ...
    /**
     * Busca una página por base
     * 
     * @param base a buscar
     * @return page encontrada.
     */
    @RequestMapping(value = "/pages/{base}", method = RequestMethod.GET)
    public ResponseEntity<?> getPage(@PathVariable final String base) {
        final Page p = this.pageRepository.findByBase(base);
        return new ResponseEntity<>(p, HttpStatus.OK);
    }
