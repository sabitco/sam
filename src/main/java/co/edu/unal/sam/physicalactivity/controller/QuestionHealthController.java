...
public class QuestionHealthController {
    ...
    public ResponseEntity<Void> createQuestionHealth(@Valid @RequestBody QuestionHealth questionHealth) {
        questionHealth = this.questionHealthRepository.save(questionHealth);
        // Set the location header for the newly created resource
        HttpHeaders responseHeaders = new HttpHeaders();
        URI newQuestionHealthUri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
    ...
    public ResponseEntity<?> getQuestionHealth(@PathVariable Long questionHealthId) {
        this.verifyQuestionHealth(questionHealthId);
        QuestionHealth p = this.questionHealthRepository.findOne(questionHealthId);
    ...
    public ResponseEntity<Iterable<QuestionHealth>> getAllquestionHealths() {
        Iterable<QuestionHealth> allquestionHealths = this.questionHealthRepository.findAll();
    ...
    public ResponseEntity<Void> updateQuestionHealth(@RequestBody QuestionHealth questionHealth, @PathVariable Long questionHealthId) {
        this.verifyQuestionHealth(questionHealthId);
    ...
    public ResponseEntity<Void> deleteQuestionHealth(@PathVariable Long questionHealthId) {
        this.verifyQuestionHealth(questionHealthId);
        this.questionHealthRepository.delete(questionHealthId);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    protected void verifyQuestionHealth(Long questionHealthId) throws ResourceNotFoundException {
        QuestionHealth questionHealth = this.questionHealthRepository.findOne(questionHealthId);
        if (questionHealth == null) {
            throw new ResourceNotFoundException("QuestionHealth with id " + questionHealthId + " not found");
        }
    }
